...
NSString *const MyManagedObjectContextSaveDidFailNotification = @"MyManagedObjectContextSaveDidFailNotification";
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    CurrentLocationViewController *vc1 = tabBarViewControllers[0];
    vc1.managedObjectContext = self.managedObjectContext;
    
    UINavigationController *vc2 = tabBarViewControllers[1];
    LocationsViewController *locationsVC = vc2.viewControllers[0];
    locationsVC.managedObjectContext = self.managedObjectContext;
    
    MapViewController *vc3 = tabBarViewControllers[2];
...
- (void)customizeAppearance {
    ...
    [UITabBar appearance].tintColor = [UIColor colorWithRed:255/255.0 green:238/255.0 blue:136/255.0 alpha:1.0];
}

#pragma mark - handle fatal Core Data Error

- (void)listenForFatalCoreDataNotifications {
    [[NSNotificationCenter defaultCenter] addObserverForName:MyManagedObjectContextSaveDidFailNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        [[self viewControllerForShowingAlert] presentViewController:alert animated:YES completion:nil];
    }];
}
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "com.Hnf.CoreDataPractice" in the application's documents directory.
